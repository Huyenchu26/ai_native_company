...
description: >-
  ...
  [EXCLUSION] KHÔNG setup product/pricing/variant/sync → vibe-eu-opc-mer-catalog; KHÔNG chạy ads/creative/script → Growth (vibe-eu-opc-grw-fb-ads / vibe-eu-opc-grw-creative); KHÔNG clearance GPSR / IP-TM / Responsible Person → vibe-opc-pod-backoffice-compliance.
...
- **Fail-closed**: thiếu log ID / không tìm thấy bản ghi / status ≠ PASS → **BLOCK publish**, set `need_review=true`, escalate `vibe-opc-pod-backoffice-compliance`. Mặc định BLOCK khi không xác nhận được.
...
- Upstream: `vibe-eu-opc-mer-catalog` (blueprint/variant/pricing), `vibe-opc-pod-backoffice-compliance` (GPSR clearance + Responsible Person)
